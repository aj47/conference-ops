...
Organizers can also start from a clean account instead of seed data. The guided launch studio supports four starter blueprints, a bounded room/track CSV import, or a protected Airtable preparation handoff. It initializes the selected venue and track structure plus a private CFP draft, routed review lanes, hotel-stay and flight-reimbursement forms, profile/slides/calendar tasks, decision and reminder templates, scheduled reminder rules, and public widgets. **Program setup** adds an event brand kit, safe persona previews, explicit track routing, scoring plans, persistent onboarding tasks, participant resources, personalized message preview/test-send, and a supervised copilot that previews impact before deep-linking to a human-controlled workflow. The Schedule workspace can draft conflict-free placements and undo that draft; the speaker portal leads with one next action; and Control Room presents Now / Next / Proof with a client activity timeline.

The current product boundary and the evidence-backed comparison with Sessionboard's participant and organizer workflows are documented in [docs/SESSIONBOARD-PARITY.md](docs/SESSIONBOARD-PARITY.md). The v0.3 assessment package includes the [KillMySaaS evidence map](docs/KILLMYSAAS-EVALUATION.md), [release audit](docs/RELEASE-AUDIT-0.3.0.md), and [pilot experience guide](docs/PILOT-WOW-GUIDE.md).

## Prospective-client pilot

The persistent organizer pilot is a separate, non-demo Workers.dev environment. Each prospect creates and verifies an account, receives an isolated organization, and launches an event from a starter blueprint or validated structure import. The in-product Control Room includes a guided event loop, safe persona previews, a nine-step trial runway, a Now / Next / Proof cockpit, and links to the bundled [Organizer Trial Guide](public/conference-ops-organizer-trial-guide.pdf).
