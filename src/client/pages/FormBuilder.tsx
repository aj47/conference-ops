...
function RichTextField({ value, onChange, label, hint }: { value: string; onChange: (value: string) => void; label: string; hint?: string }) {
  return (
    <Field label={label} hint={hint}>
      <div className="rich-field">
        <div className="rich-field__tools" aria-hidden="true"><strong>B</strong><em>I</em><u>U</u><span>• list</span><span>↗ link</span></div>
...
function AddFieldDialog({ onAdd, onClose }: { onAdd: (field: FormField) => void; onClose: () => void }) {
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={onClose}>
      ...
      >
        ...
        <div className="drawer__body form-stack">
          <Field label="Question label"><input autoFocus required value={label} onChange={(event) => setLabel(event.target.value)} placeholder="What should reviewers know?" /></Field>
          <Field label="Answer type"><select value={type} onChange={(event) => setType(event.target.value as FormFieldType)}>{Object.entries(fieldTypeNames).map(([value, name]) => <option key={value} value={value}>{name}</option>)}</select></Field>
...
function FieldRows({ fields, onChange, participant = false }: { fields: FormField[]; onChange: (fields: FormField[]) => void; participant?: boolean }) {
  ...
  return (
    <section className="builder-section">
          ...
          return (
            <article className="field-row" key={field.id}>
              ...
              <div className="field-row__main">
                ...
                <div className="field-row__meta"><span>{fieldTypeNames[field.type]}</span>{field.type === "long_text" && <span>Max 5,000</span>}{field.condition && <span className="condition-badge"><Braces size={11} /> When format = {field.condition.value}</span>}</div>
              </div>
              <Toggle checked={field.required} disabled={locked} label="Required" onChange={(required) => onChange(fields.map((item) => item.id === field.id ? { ...item, required } : item))} />
      ...
      {adding && <AddFieldDialog onClose={() => setAdding(false)} onAdd={(field) => onChange([...fields, field])} />}
    </section>
  );
}

export function FormBuilder() {
  const { builder, updateBuilder, replaceBuilderFields, saveBuilder, publishBuilder, setNotice } = useWorkspace();
  ...
  const copyLink = async () => {
    try { await navigator.clipboard.writeText(`${window.location.origin}/submit/ai-engineer-summit-2026`); } catch { /* Clipboard may be unavailable in a test browser. */ }
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <button type="button" className="button button--quiet" onClick={() => navigate("/submit/ai-engineer-summit-2026?preview=draft")}><Eye size={16} /> Preview draft</button>
        ...
        <div className="builder-workspace">
          ...
          {step === "welcome" && (
            <div className="builder-panel">
              ...
              <RichTextField label="Welcome message" value={builder.welcomeMessage} onChange={(welcomeMessage) => updateBuilder({ welcomeMessage })} hint="Links to terms, FAQs, and speaker resources are supported." />
            </div>
          )}
          ...
          {step === "proposal" && (
            <div className="builder-panel">
              ...
              <RichTextField label="Description & instructions" value={builder.proposalInstructions} onChange={(proposalInstructions) => updateBuilder({ proposalInstructions })} />
          ...
          {step === "participants" && (
            <div className="builder-panel">
              ...
              {!builder.collectParticipants ? <InlineAlert tone="warning">Participant collection is disabled in Submission setup. This public step will be removed.</InlineAlert> : <>
                ...
                <RichTextField label="Description & instructions" value={builder.participantInstructions} onChange={(participantInstructions) => updateBuilder({ participantInstructions })} />
          ...
          {step === "rules" && (
            <div className="builder-panel">
              ...
              <section className="builder-section compact-section">
                ...
                <Field label="Close date and time" hint="Setting this enables draft reminder emails."><input type="datetime-local" value={builder.closeDate} onChange={(event) => updateBuilder({ closeDate: event.target.value })} /></Field>
              </section>
              <section className="builder-section compact-section">
          ...
          {step === "confirmation" && (
            <div className="builder-panel">
              ...
              <RichTextField label="Success page message" value={builder.successMessage} onChange={(successMessage) => updateBuilder({ successMessage })} hint="Shown after the submission transaction and confirmation email job are created." />
              <div className="confirmation-preview"><span className="confirmation-preview__check"><Check size={28} /></span><p className="eyebrow">Submission S-1086 received</p><h3>You’re in the review queue.</h3><p>{builder.successMessage}</p><button type="button" className="button button--primary">Continue to portal <ArrowRight size={15} /></button>{builder.autoRedirect && <small>Continuing automatically in 10 seconds…</small>}</div>
            </div>
          )}

          {step === "notifications" && (
            <div className="builder-panel">
              <div className="builder-panel__title"><span>07</span><div><h2>Notify, verify, publish.</h2><p>Submission confirmation is required. Admin alerts stay deliberately optional.</p></div></div>
              ...
              <section className="notification-row"><div><BellRing size={20} /><span><strong>Admin alerts</strong><small>Notify selected organizers for new or updated submissions.</small></span></div><span className="priority-note">Nice to have</span><Toggle checked={builder.adminAlertsEnabled} onChange={(adminAlertsEnabled) => updateBuilder({ adminAlertsEnabled })} label={builder.adminAlertsEnabled ? "Enabled" : "Disabled"} /></section>
              {builder.adminAlertsEnabled && <Field label="Recipients"><input value={builder.adminRecipients.join(", ")} onChange={(event) => updateBuilder({ adminRecipients: event.target.value.split(",").map((value) => value.trim()).filter(Boolean) })} /></Field>}
              ...
              <button
                ...
                disabled={!Object.values(completed).every(Boolean) || publishing}
                onClick={async () => {
                  setPublishing(true);
